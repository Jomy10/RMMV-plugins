...
class RTBS_Manager {
  ...
  playerAttacks() {
    ...
    let event = Jomy.eventDetection.getFirstEvent()
    if (event != null) {
      for (let line of event.event().pages[0].list) {
        ...
        for (let param of line.parameters) {
          ...
          if (comment.getKey() == "rtbs_enemy_id") {
            ...
            enemy.getsAttacked($rtbs_player.atk);
...
(function() {
  ...
  function getEnemy(_event) {
    ...
    $rtbs_manager.addEnemy(new RTBS_Enemy(id , health, attack, speed, _event));
  }
  ...
  Scene_Map.prototype.onMapLoaded = function() {
    ...
    $rtbs_player.rtbs = {};
    $rtbs_player.rtbs.lastAttack = 0;
    $rtbs_player.rtbs.speed = 1000;
